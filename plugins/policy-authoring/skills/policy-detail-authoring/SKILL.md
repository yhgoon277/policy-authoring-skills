...
3. **할루시네이션 0** — 모든 값은 근거로 추적되는 사실. **표·수치는 원문을 직접 세어** 확인(추론 금지). 확실치 않으면 값을 만들지 말고 `field_review`로 플래그. **유일한 예외 = 라-축(P5 일반정책 추론)**: 사용자 명시 요청 시에만, 그때도 항상 붉은 배지+근거(무요청 자동일반화 금지).
4. 정형 골격·번역투 금지.
...
- **다 = to-be 미정** (as-is 근거 없이 to-be가 설계 중) → 배지 + 질문리스트. (값 비움 — 추론 안 함)
- **라 = 일반정책 추론 (opt-in)** (as-is 무근거 빈칸을 **사용자 명시 요청 시에만** 통신·글로벌 앱/웹 일반 관행으로 *제안*) → **항상** 붉은 배지 + `source_note` `[일반정책 근거·검토필요]` 프리픽스(근거 1줄). **새 필드 없이** `field_review`+`source_note` 재사용(render 변경 0).

> **P5 가드(제로추론 기본 불변)**: 라-축은 **요청 시 opt-in**이고 **항상 확인+배지+근거**다. 무요청 자동 일반화는 금지 — 뱃지 없이 본문에 들어가는 라-값은 날조와 동급 결함이다(무뱃지 라-값 경로 0). 요청 없으면 라가 아니라 다(값 비움).

분류 기준·플래깅 경계·**라-축 컨벤션**·7항목 팩트체크 체크리스트 → **[references/factcheck-and-tobe.md](references/factcheck-and-tobe.md)**.
...
## 다른 스킬과의 연계
- 첫 접촉의 gap-fill 오버레이(P3 제로추론 기본 / P5 라-축 opt-in)는 `policy-intake-router`가 이 스킬을 가리킨다.
