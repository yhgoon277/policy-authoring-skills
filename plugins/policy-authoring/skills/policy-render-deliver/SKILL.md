...
python3 tools/build_deliverable.py --spec=<입력 spec.json> --source=<원천 HTML> \
    --out-dir=<dir> [--target-code=<R5코드>] [--gate=<커스텀 게이트; 미지정 시 번들 자동>]
...
- **최종 산출물 = 5원칙을 통과한 HTML+JSON 한 쌍**(`<원천>_deliverable.html` + `<원천>_spec.json`, out-dir에 생성). 이 한 쌍이 이 스킬의 결론이다.

### R2 입력 게이트 — 기본 번들(별도 파일 불필요)
R2는 **번들 게이트 `validate_nc_input.py`(디자인팀 업로드 게이트 무수정 이식본)로 항상 기본 실행**된다 — 경로 지정 없이도 실질 검증(NA 없음). 디자인팀이 게이트를 갱신하면 `--gate=<경로>`로 override. spec이 게이트를 통과 못 하면(usecase_id 공란 등) **FAIL이 아니라 BLOCKED(gate_authoring)** — 플러그인은 spec을 날조로 채워 통과시키지 않고 사람 저작을 요청한다.

### R5 미등록 도메인 — 대화형 등록
R5가 `target_unresolved`(BLOCKED)면 **권위표 `tools/domain_codes.md`에 그 도메인이 없다**는 뜻이다. 조용히 두지 말고 사용자와 등록한다:
1. `domain_codes.md`(권위표 SSOT) 현재 목록을 제시하고, `domain_code_map.suggest_code('<도메인명>')`으로 코드 후보를 제안한다(최종 결정은 사람).
2. 사용자가 코드를 확정하면 한 행 추가:
   `python3 -c "import domain_code_map as d; d.add_domain('<도메인명>','<코드>', alias='<현행코드>')"`
3. `build_deliverable`/`run_acceptance` 재실행 → R5 즉시 해소. 권위표는 repo 1급 자산 → **커밋해 팀 공유**.
...
python3 tools/run_acceptance.py --source=<원천> --spec=<spec> --deliverable=<배포> [--target-code=..] [--gate=커스텀]
# R2 게이트는 미지정 시 번들 validate_nc_input.py로 자동 실행(NA 없음).
```
