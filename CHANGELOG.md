...
- `rebuild_policy_from_source` 이름 크로스워크 — 입력 spec의 bake ID 접미 `명 (PI-…)`·원천 제목의 검토 배지를 정규화하지 않아 장식이 다르면 이름 매칭 전건 미스 → 가산 보강(F3) 무력화(F6, 통합허브 실측 121/121 미스·rich 필드 전부 소실). `_match_key` 장식 제거 매칭으로 해소.
...
### Added
- `tests/test_domain_code.py::ActorPrefixRelabel`·`DictKeyRelabel`, `tests/test_rebuild_policy.py::EnrichFieldCarry`·`DecoratedNameCrosswalk`, `tests/test_compare_fidelity.py::PgListDetailDecorations` — 다섯 수정의 회귀 가드.
