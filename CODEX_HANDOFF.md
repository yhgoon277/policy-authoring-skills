...
- **스킬 10종**(모두 SKILL.md 포맷): `policy-intake-router`, `policy-authoring-setup`, `policy-hierarchy-decomposition`, `policy-naming-readability`, `policy-detail-authoring`, `policy-integrity-audit`, `policy-render-deliver`, `policy-nc-studio-gate`, `policy-html-json-check`, `policy-workflow-orchestration`
...
```
**팀/레포 스코프(대안):** 위 10개 폴더를 작업 레포의 `.agents/skills/`에 체크인.
...
| # | 테스트 | 방법 | 기대 결과 |
|---|---|---|---|
| **A. 인식** | 10개가 보이나 | `/skills` | `policy-*` 10개 노출, 이름·설명 표시 |
...
3. **감사 실행:**
   ...
   ```
   기대: A~L 불변식 리포트, STRUCTURAL/SEMANTIC 카운트, 실측 카운트. (정합한 최소 스펙이면 STRUCTURAL 0)
4. **렌더 실행:**
   ```bash
...
1. **README/INSTALL에 "Codex" 설치 섹션 추가**: "스킬 폴더 10개를 `~/.agents/skills/`(개인) 또는 레포 `.agents/skills/`(팀)에 둔다. `.claude-plugin/`은 Claude 전용이라 Codex는 무시. 스크립트는 Codex 샌드박스/터미널에서 실행." (현 문서는 claude.ai·Claude Code 2종만 안내 → Codex 포함 3-way로)
2. **Codex 번들 추가 검토**: `dist/policy-authoring-skills-codex.zip`(10개 스킬 폴더 flat → 풀어서 `.agents/skills/`에 복사) + `dist/CODEX_HOWTO.txt`. 폴더 복사가 번거로우면 유용. (claude.ai용 기존 zip은 유지)
...
- 스킬들이 **서로 이름으로 참조** → 10개 다 설치해야 완전 동작.
